...
class Controller:
    ...
    def save_and_predict(self, post_details:SavePredict, db:Session):
        ...
        try:
            ...
            click_id = create_record(db=db,post=post_data)


            if click_id != None:
                #cluster search
                cluster_found_ack, cluster_node_id = self.cluster_controller.find_cluster(page_id=page_id,points=cordinates)
                if cluster_found_ack == True:
                    cluster_index = self.cluster_controller.find_cluster_index(page_id=page_id,click_id=cluster_node_id)
                    return json.dumps(SavePredictResponseData(cluster_idx=int(cluster_index),is_new=False).__dict__)
                else:
                    #cluster creation
                    cluster_create_ack = self.cluster_controller.create_cluster(page_id=page_id,click_id=click_id,points=cordinates)
    ...
    def predict(self, post_details:JustPredict):
        ...
        try:
            ...
            if cluster_found_ack == True:
                cluster_index = self.cluster_controller.find_prediction_cluster_index(page_id=page_id,click_id=cluster_node_id)
